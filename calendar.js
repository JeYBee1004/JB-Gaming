...
(function() {
  // ---------- Config ----------
  const PHONE = '+254710709997';
  const POCHI_NOTE = 'Pochi La Biashara: +254710709997';
  ...
  function renderCalendar(year, month) {
    ...
    for (let d = 1; d <= last.getDate(); d++) {
      ...
      const isFullyBooked = bookedConsoles.length >= 2;
  ...
  function updateSelectedInfo() {
    if (!selectedDate) {
      selectedInfo.innerHTML = '<p class="small">No date selected</p>';
      return;
    }
  ...
  function adminMarkSelectedAsBookedLocal() {
    if (!selectedDate || !selectedConsole) {
      alert('Select date and console first.');
      return;
    }
    const iso = toISO(selectedDate);
  ...
  function showModal() {
    ...
    confirmSummary.innerHTML = `
      <div style="margin-top:10px;"><strong>Console:</strong> ${selectedConsole}</div>
      ...
      <div style="margin-top:10px;"><strong>Payment:</strong> ${POCHI_NOTE}</div>
      <div class="small" style="margin-top:10px;">After you pay via Pochi, the owner will confirm and update the calendar (server-side bookings.json).</div>
    `;
  ...
  async function init() {
    ...
    prevBtn.addEventListener('click', () => {
      const p = new Date(visibleYear, visibleMonth - 1, 1);
      renderCalendar(p.getFullYear(), p.getMonth());
    });

    nextBtn.addEventListener('click', () => {
      const n = new Date(visibleYear, visibleMonth + 1, 1);
    ...
    consoleSelect.addEventListener('change', (e) => {
      ...
      if (opt && opt.disabled) {
        selectedConsole = '';
        e.target.value = '';
        alert('That console is already booked for this date. Choose another date or console.');
      }
      checkBookButtonState();
    });
    ...
    modalConfirm.addEventListener('click', () => {
      ...
      const pendings = JSON.parse(localStorage.getItem('pendingBookings') || '[]');
      pendings.push(pending);
      localStorage.setItem('pendingBookings', JSON.stringify(pendings));
    });
  ...
  window._RELOAD_BOOKINGS = async function() {
    await loadBookings();
    renderCalendar(visibleYear, visibleMonth);
  };
